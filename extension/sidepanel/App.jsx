...
const App = () => {
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-900 text-slate-100 font-sans">
      <nav className="flex space-x-2 p-2 bg-slate-800 overflow-x-auto border-b border-slate-700 scrollbar-hide">
        {['chat', 'workspace', 'knowledge', 'history', 'voice', 'analytics'].map(tab => (
          <button 
            key={tab}
            onClick={() => setActiveTab(tab)}
            className={`px-3 py-1.5 rounded-md text-sm font-medium capitalize transition-colors ${activeTab === tab ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-700'}`}
          >
            {tab}
          </button>
        ))}
        ...
        {activeTab === 'chat' && (
          <div className="flex flex-col h-full">
            <div className="flex-1 overflow-y-auto space-y-4 mb-4">
              {messages.map((m, i) => (
                <div key={i} className={`p-3 rounded-lg max-w-[90%] ${m.role === 'user' ? 'bg-blue-600 ml-auto' : 'bg-slate-800 border border-slate-700'}`}>
                  {m.content}
                </div>
              ))}
              {loading && <div className="text-slate-500 text-sm animate-pulse">NeuroLens is thinking...</div>}
            </div>
        ...
        {activeTab === 'workspace' && (
          <div className="space-y-4">
            ...
            <div className="p-4 bg-slate-800 rounded-lg border border-slate-700">
              ...
              <div className="mt-2 text-xs text-blue-400 bg-blue-400/10 inline-block px-2 py-1 rounded">3 Active Agents</div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'knowledge' && (
          <div className="space-y-4">
            ...
            <ul className="space-y-2 mt-4">
              <li className="p-2 bg-slate-800 rounded border border-slate-700">📌 React & Tailwind Guidelines</li>
              <li className="p-2 bg-slate-800 rounded border border-slate-700">📌 Architecture Diagrams</li>
            </ul>
          </div>
        )}
