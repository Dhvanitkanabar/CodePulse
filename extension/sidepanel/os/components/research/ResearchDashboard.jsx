        ...
        <div className="h-full overflow-y-auto pb-20">
            <header className="mb-8">
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <h1 className="text-3xl font-bold text-white">Research Mode</h1>
                </div>
                <p className="text-slate-400">Extract structured intelligence, entities, and claims from the active context.</p>
            </header>

            {!isAnalysed ? (
                <div className="bg-[#111111] border border-surface-700 rounded-2xl p-12 flex flex-col items-center text-center">
                    <div className="w-20 h-20 bg-[#151515] border border-surface-700 rounded-full flex items-center justify-center mb-6">
                    ...
                    <button 
                        ...
                        className="px-8 py-3 bg-white hover:bg-[#e5e5e5] text-black rounded-xl font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                    <div className="flex justify-end gap-2 mb-6">
                        <button className="px-4 py-2 bg-transparent hover:bg-[#151515] rounded-lg text-sm font-semibold flex items-center gap-2 border border-surface-700 transition-colors">
                    ...
                    {researchData.map((section) => (
                        <div key={section.id} className="bg-[#111111] border border-surface-700 rounded-xl overflow-hidden">
                            <div 
                                onClick={() => toggleSection(section.id)}
                                className={`p-4 flex justify-between items-center cursor-pointer transition-colors ${
                                    expandedSections[section.id] ? 'bg-[#151515] border-b border-surface-700' : 'hover:bg-[#151515]'
                                }`}
                            >
                                <span className={`font-semibold ${expandedSections[section.id] ? 'text-white font-bold' : 'text-slate-200'}`}>
                                    {section.title}
                                </span>
